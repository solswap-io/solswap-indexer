...
Use this checklist for every Solana/Solswap indexer release PR from `develop` to
`master`.
...
- Confirm `https://si.soramitsu.io` routing, read-only wallet API posture, and
  Solana RPC backoff settings.
- Run `SOLSWAP_INDEXER_BASE_URL=https://si.soramitsu.io npm run smoke:production`
...
- Re-run
  `SOLSWAP_INDEXER_BASE_URL=https://si.soramitsu.io npm run smoke:production`
  and confirm health, service-info, OpenAPI, balances, assets, state, history,
  token metadata, and existing Solswap route coverage.
