...
Use this when a Solana/Solswap indexer release causes a production-impacting
issue.

## Trigger

- Roll back when health, OpenAPI compatibility, wallet balance, asset, state,
  history, token metadata, or existing Solswap routes fail production thresholds.
- Assign one incident owner and one communication owner.

## Immediate Actions

- Identify the last known-good `master` tag and deployment artifact.
- Capture failing endpoint, request IDs, upstream Solana RPC status, cache state,
  and deployment ID.
